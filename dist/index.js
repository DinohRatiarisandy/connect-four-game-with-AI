...
function score_position(board, piece) {
    ...
    for (let r = 0; r < ROW - 3; r++) {
        for (let c = 0; c < COL - 3; c++) {
            ...
            const window = Array.from({ length: WINDOW_LENGTH }, (_, i) => board[r + i][c + i]);
            score += evaluate_window(window, piece);
        }
    }
    ...
    for (let r = 0; r < ROW - 3; r++) {
        for (let c = 0; c < COL - 3; c++) {
            ...
            const window = Array.from({ length: WINDOW_LENGTH }, (_, i) => board[r + 3 - i][c + i]);
            score += evaluate_window(window, piece);
        }
    }
    return score;
...
function find_valid_columns(board) {
    let valid_cols = [];
    for (let i = 0; i < COL; i++) {
...
function take_valid_place(board, col) {
...
function have_winner(board, piece) {
