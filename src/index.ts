...
function check_winner(row: number, col: number): boolean {
...
function remove_event(): void {
    for (let ROW = 0; ROW < 6; ROW++) {
        for (let COL = 0; COL < 7; COL++) {
            ...
            cell?.removeEventListener("click", make_move);
            cell?.removeEventListener("mouseover", on_mouse_over);
            cell?.removeEventListener("mouseout", on_mouse_out);
